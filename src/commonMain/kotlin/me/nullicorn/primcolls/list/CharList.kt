 ...
 *
 * Duplicate elements are supported, meaning the same char value can be added and retrieved from
 ...
 */
class CharList private constructor(private var storage: CharArray) : PrimitiveList() {
    ...
    constructor(initialCapacity: Int = DEFAULT_CAPACITY) : this(
        CharArray(checkCapacity(initialCapacity))
    )
     ...
     *
     * @param[value] The char to insert into the list at the [lastIndex].
     */
    fun add(value: Char) {
    ...
    /**
     * Retrieves the values of each char between two indices.
     *
     ...
     */
    fun getAll(fromIndex: Int = 0, toIndex: Int = size): CharArray {
     ...
     */
    fun forEach(consumer: (value: Char) -> Unit) {
        for (i in 0 until size)
            consumer(storage[i])
    }
     ...
     * @throws[Throwable] if the consumer throws anything during an iteration.
     */
    fun forEachIndexed(consumer: (index: Int, value: Char) -> Unit) {
        for (i in 0 until size)
            consumer(i, storage[i])
    }

    /**
     * Copies the contents of the list, order preserved, into a [CharArray] with the same [size].
     *
     ...
     */
    operator fun plusAssign(value: Char) = add(value)
    ...
    companion object Factory {
        ...
        /**
         * Creates a new list containing each supplied char, in the same order that they are
         * supplied.
         *
         ...
         *
         * ```kotlin
         * CharList(initialCapacity = contents.size).addAll(*contents)
         * ```
         *
         * @param[contents] the elements to copy into the list.
         */
        fun of(vararg contents: Char) = CharList(contents.copyOf())
    }
}